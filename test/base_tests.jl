...
@testset "gf_base_equality" begin
    ...
    c = ScalarWave.grid_functions.GridFunction([0,1,2], [1e-16,1e-16,1e-16])
    @test isapprox(a, c, atol=1e-15)
    d = ScalarWave.grid_functions.GridFunction([0,1,3], [0,0,0])
...
@testset "gf_base_sin" begin
    y = [1,2,3]
    a = ScalarWave.grid_functions.GridFunction([0,1,2], y)
    @test sin(a) == sin.(y)
end
